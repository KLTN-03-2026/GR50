...
### 2. Mở Ứng Dụng (5s)
Truy cập: **https://web-local-rewrite.preview.emergentagent.com**
...
### Login Test
```bash
curl -X POST https://web-local-rewrite.preview.emergentagent.com/api/auth/login \
...
### Register Test
```bash
curl -X POST https://web-local-rewrite.preview.emergentagent.com/api/auth/register \
  -H "Content-Type: application/json" \
  -d '{
    "email":"test@test.com",
    "username":"test",
