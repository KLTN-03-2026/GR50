...
import { MessageCircle, Bot, User, Search, Plus, Clock } from 'lucide-react';
...
function DoctorConversationsTab() {
  ...
  const fetchConversations = async () => {
    try {
      ...
      setConversations([
        {
          id: 'ai',
          other_user_name: 'Trợ lý AI y tế (Gemini)',
          last_message: 'Hỏi thăm sức khỏe và phân tích triệu chứng',
  ...
  const filteredDoctors = doctors.filter(doctor =>
    ...
    (doctor.specialty_name && doctor.specialty_name.toLowerCase().includes(searchTerm.toLowerCase()))
  );
    ...
    <div className="space-y-6">
                  ...
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <div className="flex-1 min-w-0">
                        <h4 className="font-semibold text-gray-900 dark:text-white truncate">{doctor.full_name}</h4>
                        {doctor.specialty_name && (
                          <p className="text-sm text-gray-600 dark:text-gray-300">{doctor.specialty_name}</p>
                        )}
                      </div>
                      {creatingChat ? (
                        <div className="animate-spin rounded-full h-6 w-6 border-2 border-teal-500 border-t-transparent"></div>
                      ) : (
                        <Button size="sm" className="bg-teal-500 hover:bg-teal-600">Chọn</Button>
                      )}
                    </div>
                  </div>
                ))}
      ...
      {loading ? (
        ...
        <div className="bg-white dark:bg-gray-800 rounded-3xl shadow-xl p-12 text-center">
          ...
          <p className="text-gray-600 dark:text-gray-300 mb-6">Bắt đầu cuộc hội thoại mới với bác sĩ để được tư vấn</p>
          <Button onClick={openDoctorSelector} className="bg-gradient-to-r from-teal-500 to-cyan-500">
            <Plus className="w-5 h-5 mr-2" />
              ...
              <div className="flex items-center gap-4">
                <div className="relative">
                  <div className="w-14 h-14 rounded-full bg-gradient-to-br from-teal-500 to-cyan-500 flex items-center justify-center">
                    {conversation.isAi ? <Bot className="w-7 h-7 text-white" /> : <User className="w-7 h-7 text-white" />}
                  </div>
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-sm text-gray-600 dark:text-gray-300 truncate">
                    {conversation.last_message || <span className="italic text-gray-400">Bắt đầu cuộc hội thoại...</span>}
                  </p>
                </div>
    ...
    <Layout>
      <div className="min-h-screen bg-gray-50 dark:bg-gray-900 p-6">
        <div className="max-w-5xl mx-auto">
          <div className="mb-8">
            <h1 className="text-3xl font-bold text-gray-900 dark:text-white mb-2">Trung tâm Nhắn tin</h1>
            <p className="text-gray-600 dark:text-gray-400">Quản lý trò chuyện với AI và Bác sĩ</p>
          </div>
          <div className="animate-in fade-in slide-in-from-bottom-4 duration-300">
            <DoctorConversationsTab />
          </div>
        </div>
      </div>
    </Layout>
