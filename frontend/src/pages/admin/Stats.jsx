...
import { 
  ...
  ArrowUpRight, ArrowDownRight, Stethoscope, Shield, Brain
} from 'lucide-react';
import { 
...
export default function AdminStats() {
  ...
  const [filters, setFilters] = useState({
    from: '',
    to: '',
    facility_id: user?.admin_type === 'FACILITY_ADMIN' ? user.facility_id : ''
  });
  ...
  const fetchStats = async () => {
    ...
    try {
      ...
      const endpoint = isSuper ? `${API}/admin/stats` : `${API}/admin/detailed-stats`;
  ...
  const appointmentData = (stats && stats.appointments) ? [
    { name: 'Hoàn thành', value: stats.appointments.completed },
    { name: 'Đang chờ', value: stats.appointments.pending },
    { name: 'Đã hủy', value: stats.appointments.cancelled || 0 },
    { name: 'Đã xác nhận', value: stats.appointments.confirmed || 0 },
  ] : [];

  const consultationTypeData = (stats && stats.appointments) ? [
    { name: 'Online', value: stats.appointments.online || 0 },
    { name: 'Trực tiếp', value: stats.appointments.offline || 0 },
  ] : [];
        ...
        <div className="max-w-7xl mx-auto space-y-8">
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
            <div>
              ...
              <p className="text-gray-500 dark:text-gray-400 mt-1">Theo dõi hiệu suất vận hành MediSched AI</p>
            </div>
          ...
          <AnimatePresence mode='wait'>
            {stats && (
              <motion.div 
                ...
                className="space-y-8"
              >
                {stats.admin_type === 'SUPER_ADMIN' ? (
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                    <StatCard 
                      title="Tổng Cơ sở Y tế" 
                      value={stats.system_overview?.total_facilities || 0}
                      icon={<Building2 />}
                    ...
                    <StatCard 
                      title="Admin cơ sở" 
                      value={stats.system_overview?.total_admins || 0}
                      icon={<Shield />}
                    ...
                    <StatCard 
                      ...
                      value={stats.system_overview?.total_accounts || 0}
                      icon={<Users />}
                    ...
                    <StatCard 
                      ...
                      value={stats.system_overview?.api_status?.toUpperCase() || 'OFFLINE'}
                      icon={<Activity />}
                  ...
                  <>
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                      ...
                      <StatCard 
                        title="Tư vấn AI" 
                        value={stats.ai?.total_sessions || 0}
                        icon={<Activity />}
                          ...
                          <div className="space-y-6">
                             <div className="flex justify-between items-end">
                                <div>
                                   ...
                                   <p className="text-2xl font-bold">{stats.ai?.total_sessions || 0}</p>
                                </div>
                                <div className="text-right">
                                   <p className="text-sm text-gray-500">Đặt lịch thành công</p>
                                   <p className="text-2xl font-bold text-teal-600">{stats.ai?.converted_to_booking || 0}</p>
                                </div>
                             </div>
